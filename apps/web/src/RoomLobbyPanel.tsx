        ...
        <div className="mr-12 flex shrink-0 items-center gap-2">
          ...
          {connected && <div className="relative">
            ...
            <div className={`${roomMenuOpen ? 'block' : 'hidden'} absolute right-0 top-[calc(100%+0.5rem)] z-50 min-w-32 rounded-xl border border-white/15 bg-slate-950/95 p-1.5 shadow-2xl`}>
              <button className="min-h-11 w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-rose-200 transition hover:bg-rose-300/10 disabled:cursor-not-allowed disabled:opacity-40" disabled={roomExitBusy} onClick={onRequestRoomExit} type="button">
...
function SeatCard({
  ...
  const accessibleName = seat.occupied
    ? `${seat.seatNumber}. ${seat.name}${seatStates.length > 0 ? `，${seatStates.join('，')}` : ''}`
    : `${seat.seatNumber}. 空座位`
