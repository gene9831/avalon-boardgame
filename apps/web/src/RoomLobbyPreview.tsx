...
function LobbyPreviewScenario({ scenarioID }: { scenarioID: LobbyPreviewScenarioID }) {
  ...
  const [reconnectMode, setReconnectMode] = useState<'automatic' | 'manual'>('automatic')
  const preview = useMemo(() => buildLobbyPreviewState({
    scenarioID, playerCount, reconnectMode, seatChangeTargetID, startPending,
  }), [playerCount, reconnectMode, scenarioID, seatChangeTargetID, startPending])
  const model = useMemo(() => buildRoomScreenModel({
    ...
    manualReconnectAvailable: preview.manualReconnectAvailable,
    startPending,
  }), [preview, startPending])
    ...
    <div className="room-lobby-preview">
      ...
      <aside aria-label="开发预览控制" className="room-lobby-preview__controls">
        ...
        {reconnectScenario && (
          <fieldset>
            <legend>重连展示</legend>
            <button aria-pressed={reconnectMode === 'automatic'} onClick={() => setReconnectMode('automatic')} type="button">自动重连中</button>
            <button aria-pressed={reconnectMode === 'manual'} onClick={() => setReconnectMode('manual')} type="button">可手动重连</button>
          </fieldset>
        )}
