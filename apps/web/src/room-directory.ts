...
export function canJoinRoom(room: Pick<AvalonRoomSummary, 'status' | 'occupiedPlayerIDs' | 'players'>) {
  return room.status === 'lobby' && (room.occupiedPlayerIDs?.length ?? 0) < (room.players?.length ?? 1)
}
