/**
 * Temporary compatibility entry point for App and the pre-Task-6 previews.
 *
 * Task 7 migrates the live controller to the public RoomScreen props. Do not
 * add new consumers here: production scene rendering belongs to RoomScreen.
 */
...
export interface LegacyRoomScreenProps {
...
export function LegacyRoomScreen({ model, actions, diagnosticsMode, identityConfirmation, identityRecognition, stageAccessory, tools }: LegacyRoomScreenProps) {
  ...
  return (
    <div className="size-full" data-identity-confirmation-state={identityConfirmation?.state} data-identity-recognition-scene={identityRecognition?.scene.type} data-identity-recognition-state={identityRecognition?.state} data-room-mode={model.mode} data-room-screen="true">
      <RoomLayout
        chrome={{
        ...
        stage={(
          <>
            {model.numPlayers === null ? (
              ...
              <RoomStage ariaLabel={`${model.numPlayers} 人游戏圆桌`} center={center} layout={snapshot.stageLayout} players={model.players} renderPlayer={(player, layout) => (
                <RoomPlayerSeat layout={layout} onActivate={() => actions.onActivatePlayer(player.playerID)} player={identityRecognition === undefined ? player : applyLegacyRoomIdentityRecognitionSeat(identityRecognition, player)} />
              )} />
            )}
