...
test('renders the standalone default stadium model without room chrome', async ({ page }) => {
  ...
  const stage = page.getByLabel('跑道玩家矩形布局画布')
  ...
  await expect(stage.locator('.player-rect')).toHaveCount(5)
...
test('restores a valid model and diagnostics from the URL', async ({ page }) => {
  await page.goto('/stadium-layout-lab.html?maxStageWidth=240&maxStageHeight=800&players=10&avatarSize=56&minimumGap=4&diagnostics=1')
  const stage = page.getByLabel('跑道玩家矩形布局画布')
  await expect(stage).toHaveAttribute('data-layout-status', 'ready')
  await expect(stage).toHaveAttribute('data-shape', 'stadium')
  await expect(stage.locator('.player-rect')).toHaveCount(10)
...
test('canonicalizes partial, noncanonical, and oversized URL state without changing the resolved model', async ({ page }) => {
  ...
  const stage = page.getByLabel('跑道玩家矩形布局画布')
  await expect(stage).toHaveAttribute('data-layout-status', 'ready')
  await expect(stage.locator('.player-rect')).toHaveCount(7)
  await expect(page).toHaveURL('/stadium-layout-lab.html?maxStageWidth=386&maxStageHeight=482&players=7&avatarSize=56&minimumGap=4&diagnostics=1')
})
...
test('keeps SVG seat and gap labels readable when a large stage is scaled on mobile', async ({ page }) => {
  ...
  await expect(page.getByLabel('跑道玩家矩形布局画布').locator('.seat-index').first()).toHaveJSProperty('textContent', '0')
...
test('keeps numeric settings at an iOS-safe text size', async ({ page }) => {
  ...
  expect(inputFontSizes).toHaveLength(4)
...
test('keeps the settings panel non-modal and supports every close/reset path', async ({ page }) => {
  ...
  await expect(page.getByLabel('跑道玩家矩形布局画布').locator('.player-rect')).toHaveCount(8)
  await page.getByRole('switch', { name: '显示诊断' }).uncheck()
  await expect(page.locator('.gap-diagnostic, .occupied-bounds')).toHaveCount(0)
