...
function buildFixedTrackPlacement(
  input: StadiumPlayerLayoutInput,
  centerlineWidth: number,
  straightLength: number,
  targetGap: number,
): FixedTrackPlacement | null {
  const rectangleSize = 2 * input.avatarSize
...
function solveAtStraightLength(
  ...
  const compensatedTarget = Math.max(input.minimumGap, low - 0.01)
...
function firstInternallyFeasibleTick(
  ...
  while (lowTick < highTick) {
    ...
    const placement = internallyFeasiblePlacementAtMinimumGap(input, centerlineWidth, tick / 100)
    if (placement === null) {
      lowTick = tick + 1
    } else {
      highTick = tick
...
export function solveStadiumPlayerLayout(input: StadiumPlayerLayoutInput): StadiumPlayerLayoutResult {
  ...
  const playerRectangleSize = 2 * input.avatarSize
  const centerlineWidth = input.maxStageWidth - playerRectangleSize
  const maximumStraightLength = input.maxStageHeight - playerRectangleSize - centerlineWidth
  ...
  const maximumTick = Math.floor((maximumStraightLength + VALIDATION_TOLERANCE) / 0.01)
