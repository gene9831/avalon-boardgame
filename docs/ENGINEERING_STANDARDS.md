...
- HTTP、Socket、浏览器持久化、环境变量和数据库读取等结构化运行时边界 `SHOULD` 优先使用 Zod schema 解析；跨端 schema 由 browser-safe 公共包拥有，仅限 Server 或 Web 的 schema 留在对应应用。
- 每个接收方 `SHOULD` 在边界解析一次；解析成功后的内部代码使用推导类型，不得在每层重复解析。Zod 结构校验不能代替服务端认证、授权、游戏规则或当前状态校验。
...
- 所有实际可见的 Web 文本计算字号 `MUST` 不低于 12px；仅由阿拉伯数字组成、不包含单位、符号或其他文字的独立编号可以降至 10px，并 `MUST` 通过 `data-numeric-text="true"` 显式标记。玩家姓名、状态和角色标签、分数表达式、罗马数字及开发诊断不适用例外；图标尺寸不受此限制。新增或修改字号后 `MUST` 运行 Web 字号检查。
