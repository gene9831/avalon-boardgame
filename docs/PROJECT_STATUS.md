...
| 响应式房间布局模型 | 🟡 | 独立的 `apps/ui-layout-lab` 使用 Vite、Tailwind CSS 和原生 TypeScript/DOM 管理目标布局；唯一公共入口 `solveRoundTableStageLayout({ maxStageWidth, maxStageHeight, playerCount, gap?, maxAvatarSize?, avatarSizeStep? })` 只输出舞台内部的桌面、桌心和玩家座位几何，顶栏、底栏、安全区及舞台外边距由业务外壳负责。`gap` 默认 8；头像默认从 56 开始按 8 递减，并始终以 36 为最后候选。高舞台策略为每个候选依次尝试圆形和纵向跑道；跑道按单侧座位数奇偶选择桌心通道或桌心对齐座位，完整可见包络在舞台硬边界内居中。390×844 的 10 人保留 48px 头像；402×714 使用 4px 间距和 4px 步长时，7/8 人保留 52px、9/10 人保留 48px。竖向预览底栏内容固定为 `48 + 56 + 56 = 160`，并另加底部安全留白；设备预览使用 `100dvw × 100dvh` 和无背景蒙层的悬浮设置面板，Safari 工具栏收放时会重新测量和求解；旧浏览器回退到 `100vw × 100vh`。宽舞台策略和正式 Web 迁移仍待逐步对齐，旧静态原型暂时保留。 |
...
2026-09-09 圆桌舞台 API 职责拆分与高舞台验证：独立实验室的唯一公共函数改为 `solveRoundTableStageLayout({ maxStageWidth, maxStageHeight, playerCount, gap?, maxAvatarSize?, avatarSizeStep? })`，成功结果只含 `shape`、`tabletop`、`centerPanel` 以及每个座位的 `playerSeatBounds`、`avatarRect`、`nameRect` 和 `avatarTopClearance`；顶栏、底栏、安全区与舞台外边距由业务外壳承担。`gap` 统一约束任意两个玩家座位边界的最小间距，默认 8；头像默认从 56 开始按 8 递减，36 是不可突破的最终候选，非锚点尺寸线性插值其他座位参数。三项参数都可在设置面板调整并保存到 URL；非法参数返回 `invalid-input`。返回坐标以舞台左上角为原点，完整桌面与座位包络在硬边界内居中；内部轨道、基准框和间距诊断不从公共入口导出。跑道根据单侧座位数切换桌心对齐拓扑：偶数使用对称桌心通道，奇数使用左右各一个桌心对齐座位，其余座位向上下对称紧凑放置。10 人默认参数回归对应 359×435 的 40px 跑道、366×596 的 48px 跑道、406×684 的 56px 跑道和 744×776 的 56px 圆形；386×482 在 `gap = 8` 时为 40px 跑道，在 `gap = 6` 时为 48px 跑道，`avatarSizeStep = 20` 时从 56 直接回落到 36；同一舞台使用 `gap = 4`、`maxAvatarSize = 56`、`avatarSizeStep = 4` 时，7/8 人选择 52px 跑道，9/10 人选择 48px 跑道。宽舞台暂时返回 `wide-stage-strategy-pending`。竖向业务外壳底栏内容统一为 `48 + 56 + 56 = 160`，另加 `max(8px, safe-area-inset-bottom)` 底部留白；模拟画布边框不占尺寸，设置面板 backdrop 透明且无模糊。设备模式使用 `100dvw × 100dvh` 跟随 Safari 工具栏收放，并以 `100vw × 100vh` 兼容旧浏览器。包级 Vitest 27 passed，Playwright 6 passed，build/typecheck/lint 和 `git diff --check` 均 exit 0；Playwright 仅有既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。402×714 的 8 人 52px 跑道及参数化模拟预览已经人工检查；动态工具栏变化仍需在真实 iOS Safari 复验，未执行全 workspace 测试、正式 Web 布局迁移、真实 LAN 或 PostgreSQL 验收。
