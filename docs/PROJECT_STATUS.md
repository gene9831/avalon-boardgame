...
- Web 房间页已完成统一响应式重写；大厅与游戏复用同一套座位、舞台和三段式阶段面板。房主开局位于阶段操作槽，退出/解散进入房间工具；健康连接不显示状态，连续断线 8 秒后才提供手动重连。当前最低验收对为 375×667 与旋转后的 667×375；横向舞台不可行时只显示舞台不可用，不切回竖版。
...
| 响应式房间布局模型 | ✅ | Browser-safe `@avalon/ui-layout` 统一拥有纯圆桌求解器、三模式指标、框架无关 class/CSS 变量和结构样式；Lab 与正式 Web 完整房间生命周期共用唯一实现，详细数学诊断只从 `/diagnostics` 子路径按需加载。Web 以业务画布实测宽高选择竖版、紧凑横版或普通横版，再由 `ResizeObserver` 测量最终舞台内容盒并独立调用 `solveRoundTableStageLayout`。横屏不可行只替换舞台提示，不回退竖屏外壳；玩家指针命中只覆盖头像和姓名。开发模式支持关闭/尺寸/完整边界三档诊断，显示实际视口、安全区、画布、舞台、人数、头像和形态，不读取秘密。 |
| 角色与阶段展示 | ✅ | 游戏开始后使用统一求解圆桌；固定 152px 桌心只显示最新公开任务结果和连续否决状态，唯一五轮任务进度由业务外壳在竖屏/普通横屏顶栏或紧凑横屏左侧任务轨道呈现，阶段操作常驻外壳而不进入桌心。进行中的姓名牌不常驻显示本人角色；身份开关只替换本人的装饰头像并显示 `playerView` 授权的知识，开关值继续跨刷新、房间、阶段和同浏览器标签页同步。身份辨认遮罩覆盖完整舞台但不覆盖顶栏；对局结束公开角色的既有行为保持不变。 |
| 开局身份辨认 | ✅ | `startGame` 后先进入按角色配置确定的身份辨认：全员查看自己的完整身份牌，邪恶阵营辨认其他邪恶座位，Merlin 辨认全部邪恶座位，启用成对角色时由 Percival 辨认 Merlin 与 Morgana 两个不可区分候选。幕布只覆盖统一舞台，房间顶栏和工具保持可用；确认进度与唯一确认操作位于常驻阶段面板，不再建立方向专用浮层或占用桌心。后续各幕仅参与者升幕查看 `playerView` 授权的座位标记，非参与者始终处于静态不透明幕布后。当前版本不显示倒计时、不发送自动唤醒，必须等待当前步骤全部参与者确认。 |
...
下一步：请先进行等待大厅切片的用户评审；随后单独设计身份识别，且不再使用当前 curtain。当前身份识别和后续阶段均未在本切片中重新设计。

2026-09-11 完整房间生命周期统一布局验证：browser-safe `@avalon/ui-layout` 现在同时拥有纯业务外壳指标、`solveRoundTableStageLayout`、框架无关 class/CSS 变量及 `room-shell.css` 结构契约；Lab 与 Web 共用这些正式入口，详细数学诊断仅从 `/diagnostics` 子路径按需加载。Web 删除旧 `RoomLobbyPanel`、`RoomGameShell`、`RoundTable`、`SolvedRoundTableStage` 和 `RoundTableQuestSummary`，加载、大厅、身份辨认、组队、投票、任务、刺杀与结算全部通过一个 `RoomScreen`。`ResizeObserver` 测量最终舞台内容盒；横屏不可行只替换舞台提示；房间标题只显示 ID 前七位；玩家指针命中只覆盖头像与名称；五个 44px 任务节点在自身矩形底部以至少 12px 的 `UsersRound`/`CircleX` 信息显示人数和失败阈值；固定 152px 桌心只显示公开摘要。本人已提交的投票/任务牌仍在阶段面板私下反馈，其他玩家只看到公共提交进度；身份知识偏好继续跨刷新和同浏览器标签页同步。开发模式支持关闭、尺寸和完整边界三档真实 Web 诊断，读取实际视口、安全区、画布、舞台、人数、头像尺寸、形态及同一次求解的几何，不读取凭据或隐藏游戏状态。
