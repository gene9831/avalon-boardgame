...
- Web 使用仅保存在浏览器的随机默认名称与八款装饰头像；无活动座位或唯一活动房间仍处于等待阶段时，主页 Header 用户中心可修改名称和头像，等待房间内也可点击自己的原姓名牌复用同一弹窗；正式开局后两个入口立即锁定。创建/加入直接使用当前资料，不再弹出名称确认；同一房间允许同名并以座位号区分。
...
| 玩家名称与入座入口 | ✅ | 浏览器首次使用从 24 个中世纪奇幻意象与 24 个身份词中组合随机中文名称（576 种组合），并随机选择八款装饰头像，一并保存在 `localStorage`；主页 Header 用户中心支持名称校验、头像选择和重新随机，跨标签页同步。无活动座位时资料保存在本机；唯一活动房间仍为 lobby 时，主页入口和圆桌上自己的原姓名牌都可复用同一弹窗，凭 seat credential 原子更新公开名称与头像、游戏玩家名、当前房间会话和浏览器资料，并实时同步同房间客户端。服务端队列以实际处理顺序裁决资料保存与开局竞争；开局后拒绝修改并关闭未提交草稿，断线期间可保留草稿但不能保存。移动端姓名输入使用 16px 字号避免键盘呼出时触发 iOS 自动页面放大，`sm` 起恢复 14px，同时保留用户主动缩放。创建和加入直接使用当前资料，不再询问名称；客户端和服务端共同限制 NFC/trim 后 1–24 字符。同一房间允许同名，所有公开日志使用名称加座位号区分；client ID 仍防止同一浏览器重复占座。头像采用 ImperialOctopus/avalon-printable 的八款角色图标并按 CC BY 4.0 署名，但仅作为装饰，不表达隐藏角色；用户中心的“素材与许可”可用键盘展开与折叠，折叠时不暴露许可链接。 |
...
2026-09-16 浏览器门禁与 Nightly 回归修复：PR #37 的 `Browser regression (2/2)` 失败并非资料保存或凭据回归，而是既有 E2E 仍断言“存在活动房间时主页用户中心只读”，与本分支已批准的“唯一有效活动房间仍为 lobby 时名称和头像可编辑”契约冲突；测试现会从主页实际修改名称与头像，并在返回房间及清除本地凭据后验证服务端房间资料和浏览器资料均已更新。最近两次 Nightly 的 7 人专项失败来自 9 月 14 日房间场景迁移后仍查找旧版单行任务结算文案；回放在断言前已经继续到刺杀阶段，因此修复没有只替换文案，而是分别从任务进度的第 4 次任务状态和对局记录的匿名成功/失败牌总数验证稳定的公开行为。普通 CI 新增 5 人双活跃房间与 7 人第四次任务阈值的两片 `Browser game-flow contracts`，并接入既有 branch-protected `Browser smoke` 聚合门禁，使同类 UI 契约漂移在 PR 阶段暴露。两条旧断言均先在本地复现 RED；修复后主页资料 E2E **1/1 passed**，7 人专项 **2 passed / 1 skipped**，5 人专项 **1 passed / 2 skipped**，CI 同款 `Browser regression (2/2)` **14/14 passed**；`@avalon/e2e` typecheck、workflow YAML 结构校验与 `git diff --check` 均 exit 0。Playwright 仅出现既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。本轮未改动游戏规则、服务端权威逻辑或隐藏信息边界，未执行完整 Nightly 10,002 局、真实 5–10 台设备 LAN、多房间人工隔离、PostgreSQL 重启或凭据重连验收。

2026-09-16 主页等待房间资料与移动端输入补充：在等待房间资料编辑的既有服务端契约、队列序列化和版本排序机制上，主页 Header 用户中心现在会区分“房间访问锁定”和“资料编辑锁定”。无活动座位时继续本地保存；本机唯一有效活动房间仍为 lobby 时，主页可直接修改名称和八款装饰头像，并复用 seat credential、严格响应解析、房间会话版本与浏览器资料版本写入；创建和加入其他房间仍保持锁定。房间进入 playing、活动会话不唯一或凭据状态无法确认时，主页资料入口继续只读。姓名输入在移动端使用 16px、`sm` 起恢复 14px，避免 iOS 键盘呼出时自动放大页面，同时不禁用用户主动缩放。永久回归先在旧行为上失败，再覆盖主页真实路由的等待房间校验、名称/头像提交、服务端请求体、房间会话和浏览器资料回写，以及输入响应式字号。以各工作区本地 binary 执行根脚本等价验证，共 **824 tests passed**（Game 86、UI Layout 80、Web 467、Test Support 25、UI Layout Lab 67、Server 99）；Server/Web build、全工作区 typecheck、根级等价 lint、Web 最小字号审计与 `git diff --check` 均 exit 0。Web build 仅保留 619.27 kB minified / 184.84 kB gzip chunk 超过 500 kB 的既有建议性警告，Server build 仅有约 2.4 MB 体积提示，Web 测试仅出现 Node 未配置 localStorage 文件的 ExperimentalWarning。本轮未执行 Playwright、真实移动设备键盘、5–10 台设备 LAN、多房间人工隔离、PostgreSQL 重启或凭据重连验收。

2026-09-16 等待房间资料编辑：新增严格共享资料更新契约和凭 seat credential 授权的房间端点；服务端在每房间队列内同时更新 lobby metadata、`G.players` 和持久化时间戳，只向同房间 Socket.IO 客户端广播去除 credential 的 `matchData`。每次成功响应还携带同房间严格递增的资料版本，Web 只允许较新的响应回写当前房间会话和浏览器资料；迟到的旧响应不能覆盖较新的多标签页保存，换座并发会保留最新版本，不同房间的版本不会互相比较。Web 在等待房间把当前玩家原姓名牌变为语义按钮，不添加可见提示或改变既有 class、data、style；点击后复用主页用户中心弹窗并一次提交名称与头像。成功结果同步当前房间会话、浏览器资料和后续创建/加入资料；断线时保留可编辑草稿但禁用保存，普通失败保留草稿并显示行内错误，开局先被服务端序列化时关闭弹窗并显示明确通知，阶段离开 lobby 也立即丢弃未提交草稿。保存期间锁定关闭、输入、头像和随机按钮；主页已有活动房间时的锁定行为保持不变。永久回归覆盖严格请求/响应、认证和原子更新、playing 拒绝、凭据不泄漏、同房间实时广播、版本单调性、迟到响应与换座并发保护、姓名牌语义与外观契约、弹窗异步状态、断线和开局竞争。根级 `pnpm test` 为 Game 86、UI Layout 80、Web 465、Test Support 25、UI Layout Lab 67、Server 93 passed / 6 PostgreSQL tests skipped，共 **816 passed / 6 skipped**；`pnpm build`、`pnpm lint`、`pnpm typecheck`、Web 最小字号检查与 `git diff --check` 均 exit 0。Web build 仅保留 618.67 kB minified / 184.68 kB gzip chunk 超过 500 kB 的既有建议性警告，Server build 仅有约 2.4 MB 体积提示。未执行 Playwright、真实 5–10 台设备 LAN、多房间人工隔离、PostgreSQL 重启或凭据重连验收。
