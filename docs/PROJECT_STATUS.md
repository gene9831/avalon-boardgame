...
| 响应式房间布局模型 | 🟡 | 独立的 `apps/ui-layout-lab` 使用 Vite、Tailwind CSS 和原生 TypeScript/DOM 管理目标布局；现有公共入口 `solveRoundTableStageLayout({ maxStageWidth, maxStageHeight, playerCount, gap?, maxAvatarSize?, avatarSizeStep? })` 只输出舞台内部的桌面、桌心和玩家座位几何，顶栏、底栏、安全区及舞台外边距由业务外壳负责。另有独立、DOM-free 的 `solveStadiumPlayerLayout({ maxStageWidth, maxStageHeight, playerCount, avatarSize, minimumGap })` 严格中心线模型和 `/stadium-layout-lab.html` 实验页：固定头像尺寸，以轴对齐玩家矩形的欧氏最短边界距离约束 5–10 人，圆形可行时立即返回，否则求最短纵向跑道；偶数人数固定上下锚点，奇数人数固定底部锚点，其余座位保持竖直轴对称。该模型已覆盖 0.01px 量化、两两间距、顺序、锚点、中心线、最短性和确定性，但第一阶段不替换现有房间求解器。现有竖向预览的头像降档、底栏 `48 + 56 + 56 = 160`、动态视口和无蒙层设置面板保持不变。宽舞台策略、正式 Web 迁移及两套模型的接入取舍仍待单独设计确认，旧静态原型暂时保留。 |
