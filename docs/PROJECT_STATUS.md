...
2026-09-15 身份辨认回看优化：身份确认和线索确认在权威个人阶段推进后不再显示多余的成功 Toast，失败通知与提交 pending 恢复保持不变。进行中的线索辨认现在也保留眼睛开关，可随时显示或隐藏本人角色；线索仍由当前场景的“查看线索 / 暂时隐藏”独立控制。线索参与者完成后及后续对局中，眼睛开关继续同时恢复本人角色和 `playerView` 已授权的邪恶座位或 Merlin 候选座位。无线索角色等待线索参与者时删除了割裂的辅助句，只保留阶段标题和匿名进度。Web 测试为 **62 files / 435 tests passed**；`@avalon/web` build、lint、`@avalon/e2e` typecheck 与 `git diff --check` 均 exit 0。身份专项 Playwright 为 **1/1 passed**，覆盖无成功 Toast、无线索等待文案、线索阶段身份开关、完成后恢复身份与授权线索及最终进入组队；仅出现既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。Web build 保留 612.94 kB minified / 183.16 kB gzip chunk 超过 500 kB 的建议性警告。应用内浏览器另在当前真实房间复核无线索等待态与 Percival 线索态，确认多余文案已消失，线索阶段眼睛开关可隐藏并恢复本人角色。本次未改动后端、规则或持久化；未执行真实 5–10 台设备 LAN、多房间人工隔离、PostgreSQL 进程重启或真实设备断网恢复验收。
