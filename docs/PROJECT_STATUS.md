...
| 响应式房间布局模型 | 🟡 | 独立的 `apps/ui-layout-lab` 是唯一的 Vite 业务外壳原型；公共入口 `solveRoundTableStageLayout({ maxStageWidth, maxStageHeight, playerCount, gap?, maxAvatarSize?, avatarSizeStep? })` 只输出舞台内部的桌面、桌心和玩家座位几何，顶栏、底栏、安全区及舞台外边距由业务外壳负责。高舞台适配层复用 DOM-free 严格矩形核心和 `/stadium-layout-lab.html` 数学工作台：业务碰撞尺寸为 `max(seatWidth, avatarTopClearance + avatarDiameter + 4 + nameHeight)`，返回稳定正方形座位包络；圆形可行时立即返回，否则求满足全部两两欧氏边界 gap 的最短纵向跑道。桌心保留 12px 圆形保护，完整包络受硬舞台包含并居中，圆桌基准框上限为 640px；偶数人数固定上下锚点，奇数人数固定底部锚点，其余座位保持竖直轴对称。详细诊断显示基准框、包络、放置引导、直线段、请求 gap、相邻欧氏 gap 和桌面中心补偿。当前 10 人结果为 `359×435 → 36px 跑道`、`366×596 → 36px 跑道`、`406×684 → 40px 跑道`、`744×776 → 56px 圆形`；`386×482`、`gap=4`、最大 56/步长 4 的 5–10 人为 `[44, 40, 40, 40, 40, 40]`。这些硬约束和单调档位可行性覆盖早期视觉基线。现有竖向预览的底栏 `48 + 56 + 56 = 160`、动态视口和无蒙层设置面板保持不变；宽舞台策略与正式 Web 迁移仍待单独设计确认。 |
