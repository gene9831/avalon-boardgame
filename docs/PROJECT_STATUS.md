...
| 角色与阶段展示 | ✅ | 游戏开始后使用统一求解圆桌；固定 128px 桌心只显示当前任务、阶段提示和连续否决状态，唯一五轮任务进度由业务外壳在竖屏/普通横屏顶栏或紧凑横屏左侧任务轨道呈现，阶段操作常驻外壳而不进入桌心。当前任务队伍在桌心统一使用“座位号 + 单行姓名”的竖向列表，不再复用难以区分的装饰头像；长名称截断但座位号始终可见，十人第五任务的五名队员仍可放入桌心。进行中的姓名牌不常驻显示本人角色；身份开关只替换本人的装饰头像并显示 `playerView` 授权的知识，开关值继续跨刷新、房间、阶段和同浏览器标签页同步。角色牌仅在本人主动查看时覆盖舞台；线索辨认与完成等待都保留正常圆桌，对局结束公开角色的既有行为保持不变。 |
...
| 队伍提案 | ✅ | 队长直接点击圆桌座位选择正确人数，动作转发到服务端 `proposeTeam`。预览队伍在桌心使用金色虚线姓名行，可通过行尾 `×` 取消；圆桌上的入选玩家使用金色头像环和右下角 `任务` 标签，并只执行一次约 180ms 的强调动画。 |
| 队伍投票 | ✅ | 所有玩家可独立提交 approve/reject；投票进行中公开提交者座位和 `x/n 已投票` 进度，但每张赞成/反对选择在全部提交前仍只向本人可见。确认后的任务队伍在桌心使用金色实线只读姓名行，圆桌上的队员继续显示金色头像环与 `任务` 标签。提交状态以无边框、透明背景的中性 Lucide `BadgeCheck` 显示在座位左下角，结算后原位替换为绿色赞成勾或红色反对叉；图标不参与布局、不占用姓名牌内容宽度，也不会移动头像或姓名牌。座位号保持左上角、队长皇冠保持顶部、任务标签保持右下角，避免四种标记互相遮挡；`prefers-reduced-motion` 下关闭任务队伍强调动画。中央区域同时显示通过/否决及赞成、反对总数；通过结果保留到该任务全部队员提交任务牌，否决结果保留到下一次投票开始，连续第五次否决的结果保留在最终结算。图标的完整文字含义也包含在座位可访问名称中，不只依赖颜色。 |
...
2026-09-16 任务队伍辨识优化：桌心在队伍预览、确认、投票和任务阶段统一改为紧凑的“座位号 + 姓名”竖向列表，移除容易与真实玩家混淆的重复装饰头像；预览行使用金色虚线并提供可访问的行尾取消按钮，确认后改为金色实线只读行，空缺名额显示明确占位。圆桌外围的入选队员统一增加金色头像环和右下角 `任务` 标签，投票标记移至左下角，确保与左上角座位号、顶部队长皇冠互不遮挡；任务队伍强调只执行一次约 180ms，减少动态偏好下完全关闭。永久回归覆盖姓名/座位号、长名截断、预览取消、确认只读、占位以及外围标记；Web **63 files / 446 tests passed**，build、lint、typecheck 与 `git diff --check` 均 exit 0。响应式 Playwright 的 5/7/10 人统一房间外壳用例 **1/1 passed**；应用内浏览器人工确认真实五人投票房间，以及十人第五任务的五名队员与长名称预览，未见桌心溢出或标记碰撞。Web build 仅保留 614.47 kB minified / 183.39 kB gzip chunk 超过 500 kB 的既有建议性警告；Playwright 仅出现既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。本轮未改动服务端、规则、隐藏信息或持久化，未执行真实 5–10 台设备 LAN、多房间人工隔离、PostgreSQL 重启或凭据重连验收。
