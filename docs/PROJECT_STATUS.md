...
| 响应式房间布局模型 | ✅ | 纯圆桌求解器与三模式业务外壳解析已提取到 browser-safe `@avalon/ui-layout`，Lab 和正式 Web 游戏页共用唯一实现；数学诊断只由 Lab 通过 `/diagnostics` 子路径使用。游戏页以业务画布实测宽高选择竖版、紧凑横版或普通横版，再用 `ResizeObserver` 测量最终舞台内容盒并独立调用 `solveRoundTableStageLayout`。横屏不可行只替换舞台提示，不回退竖屏外壳。玩家点击只覆盖头像和姓名，等待大厅圆桌保持不变。真实设备安全区与 5–10 台 LAN 人工验收仍待执行。 |
...
2026-09-11 正式 Web 圆桌布局迁移验证：新增 browser-safe workspace 包 `@avalon/ui-layout` 作为业务外壳参数与 `solveRoundTableStageLayout` 的唯一归属，正式 Web 与 `apps/ui-layout-lab` 共用根入口，Lab 专用诊断仅从 `/diagnostics` 子路径导出。`RoomGamePanel` 已接入真实玩家、五轮任务进度、阶段内容与操作、房间工具、身份信息和识别遮罩；五轮进度归业务外壳所有，竖版/普通横版位于顶栏、紧凑横版位于左侧任务轨道，不进入固定 152px 桌心。阶段栏在三种模式中均保留独立的 48px 标题、56px 内容和 56px 操作矩形；ResizeObserver 测量最终舞台内容盒后求解，横屏不可行只替换舞台且不切回竖屏。Q22 指针命中仅覆盖头像与名称，旋转后队伍选择保持不变；竖屏工具命中与 768×1024 用户中心底部面板另有浏览器回归。等待房间继续使用原 `RoomLobbyPanel` 路径。全 workspace `pnpm test` 为 617 passed / 6 skipped，`pnpm build`、`pnpm lint`、`pnpm typecheck` 和 `git diff --check` 均 exit 0；构建保留既有单 chunk 超过 500kB 警告（574.43kB，gzip 173.87kB）。Playwright 聚焦验证为正式 Web 响应式/交互 1 passed、身份识别 2 passed，Lab 全矩阵 26 passed，仅有既有的 `NO_COLOR`/`FORCE_COLOR` Node 警告。尚未执行真实 iOS/Android 刘海安全区、真实触控与键盘、5–10 台浏览器 LAN、多房间或 PostgreSQL 重启验收。
