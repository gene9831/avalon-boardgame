...
| 响应式房间布局模型 | 🟡 | 独立的 `apps/ui-layout-lab` 使用 Vite、Tailwind CSS 和原生 TypeScript/DOM 管理目标布局；DOM-free 的 `solveRoomLayout({ width, height, playerCount })` 已完成竖向策略，输出完整区域、桌面、轨道及每个座位的边界、头像、姓名和头像顶部预留。竖向模式采用三段式，并按 `56/48/40/36` 档位依次尝试圆形和纵向跑道；390×844 的 10 人保留 48px 头像。设备预览使用 `100svw × 100svh` 和悬浮设置面板。横向策略和正式 Web 迁移仍待逐步对齐，旧静态原型暂时保留。 |
...
2026-09-08 竖向布局纯函数与独立实验室验证：新增无 React 的 `apps/ui-layout-lab`，公开入口只接收宽、高和玩家人数并自行判断方向；当前竖向结果包含顶栏、圆桌舞台、固定阶段面板及其三个离散高度子矩形、桌面、轨道、占用包络，以及每个座位的 `playerSeatBounds`、`avatarRect`、`nameRect` 和稳定 `avatarTopClearance`。求解器不接收皇冠或状态标记尺寸；渲染示例只为实际领袖显示皇冠，状态点由头像几何派生。10 人回归保持 375×667 的 40px 跑道、390×844 的 48px 跑道、430×932 的 56px 跑道和 768×1024 的 56px 圆形；5–10 人均验证安全舞台、边界间距、桌心保护、确定性与 0.01 量化。实验室支持 `100svw × 100svh` 设备模式、URL 状态、移动底部抽屉/桌面浮层和几何诊断；横向输入明确返回 `horizontal-strategy-pending`。包级 Vitest 18 passed，Playwright 4 passed，build/typecheck/lint 和 `git diff --check` exit 0；全 workspace build/lint/typecheck exit 0，Game 88、test-support 25、Web 255、排除 PostgreSQL 的 Server 84 tests passed。完整 `pnpm test` 的非数据库路径通过，但 `.env.local` 指向的 PostgreSQL 未运行，两个 PostgreSQL 文件因本机 localhost:5432 不可达而失败；未连接或修改用户数据库。Web build 保留既有单个 558.01 kB minified / 167.88 kB gzip chunk 建议性警告；Playwright 仅有既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。390×844 模拟设备截图与设置面板已在应用内浏览器人工检查；未执行真实移动设备、正式 Web 布局迁移、真实 LAN 或 PostgreSQL 验收。
