...
| 响应式房间布局模型 | 🟡 | 横向与竖向目标规范共用同一交互原型。竖向模式采用顶栏、圆桌舞台和固定阶段面板三段式，在每个 `56/48/40/36` 头像档位中先尝试圆形、再尝试纵向跑道；跑道按包含皇冠和状态预留的实际玩家座位边界压紧普通间距，并以两处对称桌心通道分隔上下座位组。390×844 的 10 人原型现可保留 48px 头像。该模型尚未迁移到正式 Web 游戏界面。 |
