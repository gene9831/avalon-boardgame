...
- 房间场景重构已完成：控制器只产出已过滤的 `RoomScene`/对应 actions，座位与任务进度展示归入 `room-presentation.ts`，删除旧模型、中心/阶段内容、叠层和兼容入口。布局内部的身份氛围统一使用 `stageAtmosphere`，不再提供遗留的 stage accessory 覆盖面；开发预览索引新增无业务状态的加载场景，覆盖全部十种顶层 scene kind。真实房间的首次身份确认现使用 `identityConfirmation`，后续私密线索使用 `identityRecognition`，两者都从不透明遮罩开始并在本地揭示后确认；服务端 phase、step、协议与 `playerView` 安全边界不变。
