...
- [ ] 按四阶段完成其余外部输入边界整改；客户端输入继续由服务端验证，每个接收方在自己的边界解析一次：
  - [x] **阶段 1：服务端协议面与入口加固** — HTTP 允许列表、strict create/join、最小 Room detail、禁用 chat、统一秘密比较、载荷上限和固定 boardgame.io。
