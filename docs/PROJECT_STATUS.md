...
| 开发房间删除与踢人 | ✅ | `/dev/status` 确认启用后才显示主页 Token 面板；删除支持 lobby/playing/finished，踢人仅支持 lobby；连接中删除、匿名同步和延迟写入不会复活房间，快速复用座位即使复制旧公开数据也会由凭据校验拒绝旧会话；活动房间 metadata 使用按房间版本保护，延迟 fetch 不会被重新标记为当前版本，kick 和正式离座都会在旧写入之后权威落盘。Server tests: 37 passed；Web tests: 59 passed. |
...
```text
pnpm test ✅ Game 28 passed；Server 37 passed；Web 59 passed（含 PostgreSQL 集成、正式离座/解散、大厅单一连接主区域、确认对话框、单次 client stop 与接口状态映射回归测试）
