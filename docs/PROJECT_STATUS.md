...
2026-09-14 房间场景复审收口：共享圆桌现在以明确的 1-based 座位号区分玩家，组队队长在桌心使用仅显示头像与编号的本地预选槽位，投票阶段沿用已提交队伍；非队长组队时只显示“等待队长提议队伍”和所需人数，不接收未提交选择。桌心与外围头像统一使用左上座位号徽章，空座中央显示 `＋`，任务队员以桌心令牌和外围青色强调对应，不再重复显示下方人物角标。投票、任务与刺杀结算改为浏览器标签页内的公开结算队列：首次快照只建立基线，新事件按顺序逐条呈现，继续按钮仅关闭本地结算而不发送服务端 move；历史回放会截断到对应任务进度，投票/任务结算不会提前显示后续任务结果或终局全员身份，刺杀结算只揭示目标，真正的对局结果才揭示全部角色。进行中的结算仍支持查看本人身份及 `playerView` 已授权线索。最终 Sol 整分支复审及后续座位徽章复评均无 P0–P3；实际自动化验证为根级 **793 tests passed**（Web 62 files / 440 tests）、`pnpm typecheck`、`pnpm lint`、`pnpm build` 与 `git diff --check` 全部 exit 0。Vite 仅保留单个 614.10 kB minified / 183.35 kB gzip JavaScript chunk 超过 500 kB 的建议性警告。应用内浏览器人工复核覆盖 5、7、10 人竖屏及紧凑横屏的组队、投票、任务、刺杀和终局代表场景，并针对最新编号方案检查大厅、组队、投票、刺杀、终局及 36–56px 头像层级，未发现页面或内部滚动溢出。`dev/room-layout` 基础方案现视为完成；真实游戏房间的统一视觉替换不纳入本轮合入，留待后续集中实施。本次仍未执行真实 5–10 台设备 LAN、多房间隔离、PostgreSQL 重启或凭据重连人工验收。
