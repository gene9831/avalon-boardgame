...
2026-09-16 任务队伍辨识优化：桌心在队伍预览、确认、投票和任务阶段统一改为紧凑的“座位号 + 姓名”竖向列表，移除容易与真实玩家混淆的重复装饰头像；列表宽度由 152px 收敛到与外围姓名牌最大宽度一致的 112px，确认态移除预览取消按钮对应的尾部空列。预览行使用金色虚线并提供可访问的行尾取消按钮，确认后改为金色实线只读行，空缺名额显示明确占位。圆桌外围的入选队员统一增加金色头像环和头像底边水平居中的完整单行 `任务` 标签，投票标记移至左下角，确保与左上角座位号、顶部队长皇冠互不遮挡；任务队伍强调只执行一次约 180ms，减少动态偏好下完全关闭。桌心编号圆圈缩至 16px，数字使用 10px 无衬线粗体并增加与行边框的留白，队员姓名同步使用界面无衬线字体；进行中的 `已投票 n/m` 移至队员列表上方，结算票数仍保留在列表下方。永久回归覆盖姓名/座位号、长名截断、预览取消、确认只读、占位、信息顺序、外围标记及真实渲染几何；Web **63 files / 447 tests passed**，build、lint、typecheck 与 `git diff --check` 均 exit 0。相关 Playwright **5/5 passed**，覆盖 5/7/10 人统一房间外壳、任务标签居中且单行完整显示、编号直径/字体/留白、姓名字体及 112px 列表宽度与确认态尾部留白；应用内浏览器人工确认真实五人投票房间，以及十人第五任务的五名队员与长名称预览，未见桌心溢出或标记碰撞。Web build 仅保留 614.64 kB minified / 183.41 kB gzip chunk 超过 500 kB 的既有建议性警告；Playwright 仅出现既有 `NO_COLOR`/`FORCE_COLOR` Node 警告。本轮未改动服务端、规则、隐藏信息或持久化，未执行真实 5–10 台设备 LAN、多房间人工隔离、PostgreSQL 重启或凭据重连验收。
