...
| 响应式房间布局模型 | 🟡 | `apps/ui-layout-lab` 同时提供业务外壳原型和独立数学工作台。`solveStadiumPlayerLayout` 与业务入口 `solveRoundTableStageLayout` 已统一使用圆形玩家边界：边界圆半径等于头像直径，全部玩家对按圆心距约束，业务桌心保护半径固定 80px；圆形优先，否则选择最短纵向跑道，公开几何量化到 0.01px 后完整复验。386×482 下 5–8 人可使用 56px、9–10 人可使用 48px。方向由舞台宽高直接推导，宽舞台把内部求解宽度限制到短边，继续使用同一竖直轴算法并水平居中，不再存在独立宽舞台数学策略。业务 Lab 已完成竖版、紧凑横版和普通横版三模式 Grid 外壳、Lucide 图标和可换行的向上信息浮层；只有正式 `apps/web` 迁移与真实设备验收仍待单独实施。 |
