...
| 响应式房间布局模型 | 🟡 | `apps/ui-layout-lab` 同时提供业务外壳原型和独立数学工作台。独立 `solveStadiumPlayerLayout` 已迁移为圆形玩家边界：边界圆半径等于头像直径，全部玩家对按圆心距约束，并原生接收默认 0 的 `centerProtectionRadius`；圆形优先，否则选择最短纵向跑道，公开几何量化到 0.01px 后完整复验。386×482 下 5–8 人可使用 56px、9–10 人可使用 48px；56px 且保护半径 88 时 5、6 人均可行。`/stadium-layout-lab.html` 已提供保护半径 URL/控件、保护圆和真实 clearance 诊断。业务入口 `solveRoundTableStageLayout` 本阶段仍使用旧矩形核心、现有 152px 桌心范围和额外 12px 规则，尚未迁移；其既有代表结果保持不变。宽舞台策略、业务圆形迁移与正式 Web 迁移仍待单独实施。 |
