...
- 房间场景重构已完成：控制器只产出已过滤的 `RoomScene`/对应 actions，座位与任务进度展示归入 `room-presentation.ts`，删除旧模型、中心/阶段内容、叠层和兼容入口。布局内部的身份氛围统一使用 `stageAtmosphere`，不再提供遗留的 stage accessory 覆盖面；开发预览索引新增无业务状态的加载场景，覆盖全部十种顶层 scene kind。生产身份确认仍维持当前分步接入；预览原型不改变服务端流程。
...
| 角色与阶段展示 | ✅ | 游戏开始后使用统一求解圆桌；固定 128px 桌心只显示当前任务、阶段提示和连续否决状态，唯一五轮任务进度由业务外壳在竖屏/普通横屏顶栏或紧凑横屏左侧任务轨道呈现，阶段操作常驻外壳而不进入桌心。进行中的姓名牌不常驻显示本人角色；身份开关只替换本人的装饰头像并显示 `playerView` 授权的知识，开关值继续跨刷新、房间、阶段和同浏览器标签页同步。身份辨认遮罩覆盖完整舞台但不覆盖顶栏；对局结束公开角色的既有行为保持不变。 |
...
2026-09-14 房间场景复审收口：共享圆桌现在以明确的 1-based 座位号区分玩家，组队队长在桌心使用仅显示头像与编号的本地预选槽位，投票阶段沿用已提交队伍；非队长组队时只显示“等待队长提议队伍”和所需人数，不接收未提交选择。桌心与外围头像统一使用左上座位号徽章，空座中央显示 `＋`，任务队员以桌心令牌和外围青色强调对应，不再重复显示下方人物角标。投票、任务与刺杀结算改为浏览器标签页内的公开结算队列：首次快照只建立基线，新事件按顺序逐条呈现，继续按钮仅关闭本地结算而不发送服务端 move；历史回放会截断到对应任务进度，投票/任务结算不会提前显示后续任务结果或终局全员身份，刺杀结算只揭示目标，真正的对局结果才揭示全部角色。进行中的结算仍支持查看本人身份及 `playerView` 已授权线索。最终 Sol 整分支复审及后续座位徽章复评均无 P0–P3；合入前清理了仅供开发预览使用的生产 CSS 引用、重复渲染分支和 8 个不保护生产契约的冗余测试，并将端到端测试迁移到当前场景控件契约。实际自动化验证为根级 **785 tests passed**（Web 62 files / 432 tests）、`pnpm typecheck`、`pnpm lint`、`pnpm build` 与 `git diff --check` 全部 exit 0；受影响的端到端场景 18/18 通过。Vite 仅保留单个 614.11 kB minified / 183.37 kB gzip JavaScript chunk 超过 500 kB 的建议性警告。应用内浏览器人工复核覆盖 5、7、10 人竖屏及紧凑横屏的组队、投票、任务、刺杀和终局代表场景，并针对最新编号方案检查大厅、组队、投票、刺杀、终局及 36–56px 头像层级，未发现页面或内部滚动溢出。`dev/room-layout` 基础方案现视为完成；真实游戏房间的统一视觉替换不纳入本轮合入，留待后续集中实施。本次仍未执行真实 5–10 台设备 LAN、多房间隔离、PostgreSQL 重启或凭据重连人工验收。
