...
非锚点头像尺寸在相邻两档之间线性插值 `seatWidth`、`nameHeight` 和 `avatarTopClearance`。玩家边界圆的最小间距不属于头像档位，由调用方的 `gap` 统一控制；省略时全部档位都使用 8。
...
业务映射在 `playerSeatBounds` 内垂直居中头像、`nameGap = 4` 的姓名区以及皇冠顶部预留。姓名可以超出玩家碰撞圆，但保持在座位正方形内；其与桌心的安全关系由固定桌心保护圆覆盖。头像包装层绘制头像、皇冠和动态状态标记；状态标记由头像几何派生，不属于求解器输出，也不扩大稳定边界。
...
每个档位把 `min(maxStageWidth, 2 × avatarDiameter + 0.86 × 640)` 交给圆形数学模型；模型在扣除玩家圆直径后选择最大的中心线宽度，并在舞台高度内搜索圆形或最短跑道。业务层将返回中心线宽度除以 `0.86` 得到内部圆桌基准框。
