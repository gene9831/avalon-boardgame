...
业务求解器复用[跑道玩家矩形布局模型](./stadium-player-layout-model.md)的共享 DOM-free 严格矩形核心：圆形优先，圆形不可行时选择最短纵向跑道。业务层只负责把头像、姓名和皇冠顶部预留映射进核心给出的稳定正方形座位包络。
...
共享核心的稳定座位碰撞包络始终是正方形。业务层先由头像档位得到内容高度，并以两者较大值作为核心输入：

```text
businessContentHeight = avatarTopClearance
    + avatarDiameter + nameGap + nameHeight

playerCollisionSize = max(seatWidth, businessContentHeight)
```

严格核心返回的每个 `playerRect` 就是公共 `playerSeatBounds`。业务映射在该方形内垂直居中 `businessContentHeight`：`avatarRect` 位于皇冠顶部预留之后，`nameRect` 位于头像下方，`nameGap = 4`。头像包装层绘制头像、皇冠和动态状态标记；状态标记由头像几何派生，不属于求解器输出，也不扩大稳定边界。
...
碰撞以稳定方形 `playerSeatBounds` 为单位。任意两座位使用其最近边界点的真实欧氏距离（水平、垂直和对角均适用），并且**所有两两组合**都必须满足 `rectangleBoundaryGap >= gap`；中心距、只检查相邻座位或轴向分离都不能替代此约束。

圆形可行时严格核心立即选择圆形，不继续压缩圆形宽度。否则它在硬边界内求满足所有两两间距的最短纵向跑道，再依次最小化相邻视觉顺序 gap 的最大超出量和总超出量。偶数人数固定六点钟和十二点钟锚点；奇数人数固定六点钟锚点；其余座位按顺时针顺序并关于竖直中轴镜像。`gap` 是硬下限，不要求间距相等。

桌心与任一稳定座位边界必须保留 12px。实验室的几何模式将此限制画为以桌心面板外扩 12px 的圆形 `.center-panel-protection`，它是诊断层而不是额外布局语义。
...
实验室可以通过非公共详细入口取得圆桌基准框、完整占用包络、放置引导框 `placementGuide`、跑道直线段、请求的座位 gap、相邻视觉顺序的欧氏边界 gap，以及桌面中心补偿。渲染的 gap 线从每对相邻座位的真实最近边界点开始，以其欧氏距离和旋转角显示。
...
| 页面尺寸 | 传入舞台尺寸 | 头像 | 形态 |
| --- | ---: | ---: | --- |
| 375×667 | 359×435 | 36 | 跑道 |
| 390×844 | 366×596 | 36 | 跑道 |
| 430×932 | 406×684 | 40 | 跑道 |
| 768×1024 | 744×776 | 56 | 圆形 |

以上结果均使用默认 `gap = 8`。另一个参数化回归基线为页面 402×714、舞台 386×482：使用 `gap = 4`、`maxAvatarSize = 56`、`avatarSizeStep = 4` 时，5–10 人的结果为 `[44, 40, 40, 40, 40, 40]`。这些较早视觉基线因全部两两欧氏 gap、12px 桌心保护、硬边界包含、严格圆形优先/最短跑道与单调档位可行性而被覆盖；它们不是设备判断规则，也不恢复中心通道语义。
...
8. 每个稳定方形 `playerSeatBounds` 的中心必须位于返回的圆形或跑道中心线上，公共验证容差为 0.001px；偶数人数固定上下中轴锚点，奇数人数固定底部中轴锚点，其余座位竖直轴对称并保持视觉顺时针顺序。
