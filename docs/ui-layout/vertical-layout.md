...
共享坐标系、圆桌占用包络、座位局部边界、碰撞约束和安全取整以[横向布局规范](./horizontal-layout.md)为准。可在浏览器中打开[统一横竖屏交互原型](./prototypes/avalon-landscape-responsive-lab.html)检查本文算法。
...
| `insetTop`, `insetRight`, `insetBottom`, `insetLeft` | 四侧安全区 |
...
跑道不根据当前可见标记改变几何。每个玩家都使用所选档位的最坏合法状态边界：
...
所有座位都永久预留领袖皇冠和最大状态标记，以保证领袖轮换、成员选择或连接状态变化时座位不跳动。碰撞、包络和诊断显示必须使用同一个 PlayerSeatBounds，不得用仅从头像顶部开始的按钮矩形代替。
...
规范只约束输入、几何和字典序目标，不绑定正式 Web 实现的内部求解技术。交互原型使用有界、确定性的轨道候选搜索；相同输入必须返回相同结果。所有几何取整后重新执行包含、座位碰撞、桌心碰撞和补偿上限检查；失败时继续搜索或进入下一档。
...
交互原型的画布边框位于内容盒内，因此实际可用宽高各减少 2。375×667、10 人时，40 档圆形相邻座位空间不足，随后求得 40 档跑道候选：

```text
prototypeStage = 373 × 465
safeStage = 357 × 449
roundTableFrameWidth = 322
stadiumStraightLength = 57
playerOrbit = 276.92 × 333.92
tabletop = 238.28 × 295.28
roundTableFootprint = 356.92 × 415.92
standardAdjacentBoundaryGap = 8
centerAisleGap = 8.80
```

该跑道的玩家轨道高宽比约为 `1.206`。玩家中心不再沿跑道周长等距排列，而是先让八个普通相邻边界保持 8 的间距，再为两处桌心通道保留对称的最小额外空间。较大头像档位仅在安全舞台内找不到满足完整碰撞约束的圆形或跑道时才会降档。

当前交互原型的 10 人代表结果为：

| 可用尺寸 | 头像档位 | 形态 | `stadiumStraightLength` | 玩家轨道高宽比 | 普通边界间距 | 桌心通道间距 |
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| 375×667 | 40 | 跑道 | 57 | 1.206 | 8 | 8.80 |
| 390×844 | 48 | 跑道 | 172 | 1.643 | 8 | 40.14 |
| 430×932 | 56 | 跑道 | 181 | 1.621 | 8 | 8.52 |
| 768×1024 | 56 | 圆形 | 0 | 1.000 | — | — |

这些数值是当前原型在指定输入下的确定性输出，用于回归而不是新的设计常量。尤其是 390×844：安全舞台为 364×594，48 档玩家座位宽度为 96；通过按真实座位边界压紧普通间距并设置对称桌心通道，可以保留 48px 头像，不再因保守等弧分布降到 40px。
...
- 圆形或纵向跑道玩家轨道；
- 完整圆桌占用包络；
- 各玩家包含皇冠、姓名和最大状态标记预留的 PlayerSeatBounds；
- 普通相邻座位之间的边界间距线；
- 两处对称桌心通道间距线；
...
本文和交互原型是目标布局规范，不代表当前 Web 游戏界面已经采用竖向三段布局。本次设计不修改正式 Web 游戏界面，也不增加任何游戏阶段行为。
