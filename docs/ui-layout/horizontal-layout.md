...
本文定义 Avalon 房间界面的目标横向业务外壳。圆桌舞台内部不再维护独立的宽舞台几何策略；统一公共 API、坐标系、座位档位和全部方向的算法以[圆桌舞台布局规范](./round-table-stage-layout.md)为准。

“横向布局”表示布局模式已经由上层响应式模式选择器选定；本文不规定何时从其他布局切换到横向布局。当前 Web 的加载、大厅、身份辨认、全部游戏阶段和结算均采用本文的常驻阶段侧栏。

项目术语以 [`CONTEXT.md`](../../CONTEXT.md) 为准。本文使用其中定义的横向房间布局、圆桌舞台、圆桌布局基准框、桌面、玩家中心轨道、圆桌占用包络、阶段侧栏和座位呈现档位。

可在浏览器中打开 [`apps/ui-layout-lab`](../../apps/ui-layout-lab/README.md) 的 Vite 业务实验室，通过模拟尺寸与设备尺寸检查竖版、紧凑横版和普通横版业务外壳及统一圆桌算法；正式 `apps/web` 的完整房间生命周期共用这些纯布局与结构入口。

竖向模式的业务外壳由[竖向布局规范](./vertical-layout.md)定义。外壳先分配顶栏、任务轨道或阶段操作区域，再把扣除外部边距后的舞台内容盒交给统一圆桌舞台 API；两种方向不使用两个公共求解函数。

业务外壳模式只根据完整业务画布宽高选择。横版按业务高度分为紧凑横版和普通横版；圆桌舞台数学只根据已分配 `RoundTableStage.width` 与 `RoundTableStage.height` 独立判断方向，因此横版不会回退到旧的独立横向或胶囊数学算法。`375×667` 与旋转后的 `667×375` 是当前最小验收对，`568×320` 不属于支持基线。
...
7. 空间不足时只在舞台内明确显示不可用；横向业务外壳、工具和阶段侧栏保持横向，不回退竖版。

## 3. 几何总览

![普通横版房间布局几何模型](./assets/horizontal-layout-model.svg)

上图展示普通横版；紧凑横版的三列业务矩形由 6.4 节定义，圆桌舞台内部统一交给 `solveRoundTableStageLayout`。
...
> **历史方案，已废弃：** 本节参数不再作为横屏圆桌实现或验收依据。现行圆桌参数由[统一圆桌舞台规范](./round-table-stage-layout.md)定义。
...
> **历史方案，已废弃：** 横屏不再维护独立座位档位；实际档位由统一圆桌舞台求解器选择。
...
> **历史方案，已废弃：** 9.1–9.5 记录早期独立横向圆桌候选，不再是实现要求。现行实现根据 `RoundTableStage.width` 与 `RoundTableStage.height` 自动判断方向；宽大于高时把内部求解宽度限制到短边，继续使用竖直轴对称、六点钟当前玩家和视觉顺时针顺序，并将完整结果水平居中。调用方不得实现本节旧公式形成第二套数学模型。
...
> **历史方案，已废弃：** 本节旧矩形座位约束已由统一模型的玩家边界圆、固定 80px 桌心保护圆、全部两两圆边界 gap 和完整包络复验取代。
...
> **历史方案，已废弃：** 11.1–11.2 的独立普通横版和水平胶囊算法不再迁移。横向业务外壳只把最终 `RoundTableStage.width`、`RoundTableStage.height` 和玩家参数传给 `solveRoundTableStageLayout`；方向由这两个尺寸直接推导，不传 `horizontal`、`vertical` 或设备类别。宽舞台使用短边限定的同一竖向算法，空间不足返回 `no-fitting-stage-layout`。
...
本节数值属于旧独立横向圆桌候选，只保留为设计沿革，不参与当前实现和验收。横向业务外壳迁移时应以其分配出的 `RoundTableStage` 尺寸调用统一求解器，并以实际返回结果建立新基线。
...
- 统一求解器返回的圆形或纵向跑道 `placementGuide`。
...
3. `RoundTableStage.width > RoundTableStage.height` 时仍使用统一竖向算法，内部求解宽度不超过短边，完整结果水平居中。
4. 完整包络位于安全舞台内。
5. 玩家座位之间的间距不少于 `seatGap`。
6. 玩家边界圆不与固定 80px 桌心保护圆相交。
7. 完整圆桌占用包络保持居中并位于圆桌舞台硬边界内。
8. 所有玩家内容保持屏幕正向。
9. 最小交互热区符合所选档位。
...
- 当前界面没有本文定义的常驻阶段侧栏。
- 当前 Web 尚未消费统一圆桌舞台求解器。
- 当前 Web 仍包含多组针对高度和人数的位移修正；统一求解器已经使用完整包络求解替代重复预留和经验位移。

采用本文横向业务外壳并把正式 Web 接入统一求解器属于后续实现任务，必须另行制定迁移计划和回归范围；本文本身不授权修改现有游戏 UI。
