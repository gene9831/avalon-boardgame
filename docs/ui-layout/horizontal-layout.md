...
“横向布局”表示布局模式已经由上层响应式模式选择器选定；本文不规定何时从其他布局切换到横向布局。当前 Web 游戏页已经采用本文的常驻阶段侧栏；等待大厅仍使用独立布局。

项目术语以 [`CONTEXT.md`](../../CONTEXT.md) 为准。本文使用其中定义的横向房间布局、圆桌舞台、圆桌布局基准框、桌面、玩家中心轨道、圆桌占用包络、阶段侧栏和座位呈现档位。

可在浏览器中打开 [`apps/ui-layout-lab`](../../apps/ui-layout-lab/README.md) 的 Vite 业务实验室，通过模拟尺寸与设备尺寸检查竖版、紧凑横版和普通横版业务外壳及统一圆桌算法；正式 `apps/web` 游戏页已共用这些纯布局入口，等待大厅保持独立。
