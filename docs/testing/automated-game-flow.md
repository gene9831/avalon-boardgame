...
| Browser smoke | Five isolated browser profiles create, join, refresh, play, and settle a game | `pnpm test:e2e` |
| Browser matrix | Seeded complete games with 5, 6, 7, 8, 9, and 10 isolated browser profiles | `pnpm test:e2e:matrix` |

The browser suite creates a separate Playwright context for each player. This is important because a single browser profile shares Avalon identity and credentials across tabs.
...
`.github/workflows/nightly.yml` runs at 18:00 UTC (02:00 Asia/Shanghai) and can also be started manually. The scheduled master seed is `nightly-YYYY-MM-DD`; browser shards append `-<players>p`. A manual run can provide a safe custom `master_seed` and property run count. Failed property logs and Playwright traces/screenshots are retained as workflow artifacts for 30 days.

The nightly workflow runs entirely on GitHub-hosted runners. No scheduled process is installed on a developer computer.

## Scope boundary

Automation is a regression gate, not a substitute for real LAN acceptance. Continue to use [the LAN multiplayer acceptance guide](lan-multiplayer-acceptance.md) for multiple physical devices, real network interruptions, two-room concurrency, and deployment-environment restart exercises.
