...
Regular CI uses the pnpm 11 native `pnpm/setup` action to install pnpm and Node without the npm-based self-installer used by the legacy action. Dependency installation remains an explicit step for readable timing and failure logs. `Browser smoke execution` runs only the two `@smoke` acceptance flows; the remaining browser suite is split across `Browser regression (1/2)` and `Browser regression (2/2)`. The shards keep one Playwright worker each so multi-browser scenarios do not contend inside a runner, while separate runners reduce the browser gate's wall-clock critical path. The branch-protected `Browser smoke` context is a lightweight aggregate that succeeds only after the smoke execution and both regression shards succeed, so existing protection cannot bypass a new shard.
