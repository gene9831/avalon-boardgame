...
| PR browser smoke | Atomic create-and-enter, lowest-empty automatic joins, seat-independent ownership, and a five-player rejected-team game | `pnpm test:e2e:smoke` |
| PR browser regression | Dialogs, development controls, interrupted seat-change recovery across tabs, refresh reconnect, paired-role recognition/privacy, all four victory paths, and 5/7/10-player target-viewport operation | `pnpm test:e2e:regression` |
...
Regular CI uses the pnpm 11 native `pnpm/setup` action to install pnpm and Node without the npm-based self-installer used by the legacy action. Dependency installation remains an explicit step for readable timing and failure logs. `Browser smoke execution` runs only the two `@smoke` acceptance flows; the remaining browser suite is split across `Browser regression (1/2)` and `Browser regression (2/2)`. A separate two-shard `Browser game-flow contracts` matrix runs the deterministic 5-player concurrent-room scenario and both 7-player fourth-quest threshold scenarios, so UI contract drift is caught on the pull request that introduces it instead of waiting for the schedule. These jobs keep one Playwright worker each so multi-browser scenarios do not contend inside a runner, while separate runners reduce the browser gate's wall-clock critical path. The branch-protected `Browser smoke` context is a lightweight aggregate that succeeds only after smoke execution, both regression shards, and both game-flow contract shards succeed, so existing protection cannot bypass a new shard.
